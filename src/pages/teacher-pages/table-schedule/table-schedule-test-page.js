        ...
        <div className=" rounded-lg min-w-[1200px]">
          <div className="grid">
            ...
            {orderedDate.map((date, dateIndex) => (
              <div
                key={`date-${dateIndex}`}
                className="grid custom-grid-table "
              >
                <div
                  className={`col-span-6 min-h-16 p-2 ${getColorByDate(date)}`}
                >
                  {date}
                </div>
                  ...
                  if (course.day_id === dateIndex + 1) {
                    return (
                      <div
                        ...
                        className={`border p-2 md:px-3 md:py-2 rounded text-xs md:text-sm bg-opacity-60 flex flex-col justify-between hover:bg-opacity-70 overflow-hidden cursor-pointer dark:bg-opacity-100 dark:border-gray-700 ${getColorByDate(
                          date
                        )}`}
                        style={{
                          gridColumnStart: timeToCol(course.st),
                          gridColumnEnd: timeToCol(course.et),
                        }}
                      >
                        <p className="flex flex-wrap justify-between mb-2">
                          <span>{course.SUBJECT}</span>
                          <span>
                            [{course.st?.slice(0, 5)} - {course.et?.slice(0, 5)}
                            ]
                          </span>
                        </p>
                        <p>
